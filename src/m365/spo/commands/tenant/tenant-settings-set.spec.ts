...
describe(commands.TENANT_SETTINGS_SET, () => {
  ...
  const defaultRequestsSuccessStub = (): jest.Mock => {
  ...
  it('sends xml as array of strings for option excludedFileExtensionsForSyncClient',
    async () => {
  ...
  it('handles tenant settings SelectAllProperties (first \'POST\') request error',
    async () => {
  ...
  it('handles tenant settings set (second \'POST\') request error',
    async () => {
  ...
  it('should turn enums to int in the request successfully', async () => {
    const stubRequest: jest.Mock = defaultRequestsSuccessStub();
  ...
  it('shows warning when option EnableAzureADB2BIntegration is used with value true',
    async () => {
      defaultRequestsSuccessStub();
