...
describe(commands.FILE_GET, () => {
  ...
  it('retrieves and prints all details of file as ListItem object',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('retrieves and prints all details of file as ListItem object with permissions',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf('?$expand=ListItemAllFields') > -1) {
          return {
  ...
  it('uses correct API url when id option is passed', async () => {
    const getStub: any = jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
      if ((opts.url as string).indexOf('/_api/web/GetFileById(') > -1) {
        return 'Correct Url';
      }
    ...
    assert.strictEqual(getStub.mock.lastCall[0].url, 'https://contoso.sharepoint.com/sites/project-x/_api/web/GetFileById(\'0CD891EF-AFCE-4E55-B836-FCE03286CCCF\')');
  });
  ...
  it('uses correct API url when url option is passed', async () => {
    ...
    assert.strictEqual(getStub.mock.lastCall[0].url, `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFileByServerRelativePath(DecodedUrl=@f)?@f='%2Fsites%2Fproject-x%2FDocuments%2FTest1.docx'`);
  });

  it('uses correct API url when tenant root URL option is passed',
    async () => {
      const getStub: any = jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf('/_api/web/GetFileByServerRelativePath(') > -1) {
          return 'Correct Url';
      ...
      assert.strictEqual(getStub.mock.lastCall[0].url, `https://contoso.sharepoint.com/_api/web/GetFileByServerRelativePath(DecodedUrl=@f)?@f='%2FDocuments%2FTest1.docx'`);
    }
  );
