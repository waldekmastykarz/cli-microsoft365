...
describe(commands.APP_TEAMSPACKAGE_DOWNLOAD, () => {
  ...
  it('downloads Teams app package when appItemUniqueId specified',
    async () => {
  ...
  it('downloads Teams app package when appItemId specified (debug)',
    async () => {
      const mockResponse = `{"data": 123}`;
      const responseStream = new PassThrough();
      responseStream.write(mockResponse);
  ...
  it('saves the downloaded Teams package to file with name following the .sppkg file',
    async () => {
      const mockResponse = `{"data": 123}`;
      const responseStream = new PassThrough();
      responseStream.write(mockResponse);
  ...
  it('saves the app package downloaded using appItemUniqueId to the specified file',
    async () => {
      const mockResponse = `{"data": 123}`;
      const responseStream = new PassThrough();
      responseStream.write(mockResponse);
  ...
  it('saves the app package downloaded using appItemId to the specified file',
    async () => {
      const mockResponse = `{"data": 123}`;
      const responseStream = new PassThrough();
      responseStream.write(mockResponse);
  ...
  it('saves the app package downloaded using appName to the specified file',
    async () => {
      const mockResponse = `{"data": 123}`;
      const responseStream = new PassThrough();
      responseStream.write(mockResponse);
  ...
  it(`handles error when the specified app catalog doesn't exist`,
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `https://contoso.sharepoint.com/sites/appcatalog/_api/web/GetFolderByServerRelativePath(DecodedUrl='AppCatalog')/files('m365-spfx-wellbeing.sppkg')/ListItemAllFields?$select=Id`) {
  ...
  it(`handles error when the specified appItemUniqueId doesn't exist`,
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `https://contoso.sharepoint.com/_api/SP_TenantSettings_Current`) {
          return { "CorporateCatalogUrl": "https://contoso.sharepoint.com/sites/appcatalog" };
  ...
  it(`handles error when the package doesn't support syncing to Teams`,
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `https://contoso.sharepoint.com/_api/SP_TenantSettings_Current`) {
          return { "CorporateCatalogUrl": "https://contoso.sharepoint.com/sites/appcatalog" };
  ...
  it(`handles error when saving the package to file fails`, async () => {
    ...
    const writeStream = new PassThrough();
    jest.spyOn(fs, 'createWriteStream').mockClear().mockReturnValue(writeStream as any);
