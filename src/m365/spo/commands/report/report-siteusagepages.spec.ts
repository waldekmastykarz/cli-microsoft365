...
import command from './report-siteusagepages.js';
...
describe(commands.REPORT_SITEUSAGEPAGES, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.REPORT_SITEUSAGEPAGES);
  });
  ...
  it('gets the report for the last week', async () => {
    const requestStub: jest.Mock = jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
      if (opts.url === `https://graph.microsoft.com/v1.0/reports/getSharePointSiteUsagePages(period='D7')`) {
        return `Report Refresh Date,Site Type,Page View Count,Report Date,Report Period`;
      }
    ...
    assert.strictEqual(requestStub.mock.lastCall[0].url, "https://graph.microsoft.com/v1.0/reports/getSharePointSiteUsagePages(period='D7')");
