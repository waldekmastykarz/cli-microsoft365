...
describe(commands.PAGE_COLUMN_LIST, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.PAGE_COLUMN_LIST);
  });
  ...
  it('lists columns on the modern page when the specified page name doesn\'t contain extension',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf(`/_api/web/GetFileByServerRelativePath(DecodedUrl='/sites/team-a/SitePages/home.aspx')`) > -1) {
          return apiResponse;
  ...
  it('lists all information about columns on the modern page in json output mode',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf(`/_api/web/GetFileByServerRelativePath(DecodedUrl='/sites/team-a/SitePages/home.aspx')`) > -1) {
          return apiResponse;
  ...
  it('fails validation if the webUrl option is not a valid SharePoint site URL',
    async () => {
      const actual = await command.validate({ options: { webUrl: 'foo', pageName: 'home.aspx', section: 1 } }, commandInfo);
  ...
  it('fails validation if the section option is not a number', async () => {
    const actual = await command.validate({ options: { webUrl: 'https://contoso.sharepoint.com', pageName: 'home.aspx', section: 'abc' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });
