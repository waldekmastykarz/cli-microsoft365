...
describe(commands.TASK_ADD, () => {
  ...
  it('fails validation if neither the planId, planTitle, nor rosterId are provided.',
    async () => {
  ...
  it('fails validation when both planId, planTitle, and rosterId are specified',
    async () => {
      jest.spyOn(cli, 'getSettingWithDefaultValue').mockClear().mockImplementation((settingName, defaultValue) => {
        if (settingName === settingsNames.prompt) {
          return false;
  ...
  it('passes validation when valid title, planId, and bucketId specified',
    async () => {
      const actual = await command.validate({
        options: {
          title: 'My Planner Task',
  ...
  it('passes validation when valid title, planTitle, and ownerGroupId are specified',
    async () => {
      const actual = await command.validate({
        options: {
          title: 'My Planner Task',
  ...
  it('passes validation when valid title, planTitle, ownerGroupName, and bucketId are specified',
    async () => {
      const actual = await command.validate({
        options: {
          title: 'My Planner Task',
  ...
  it('fails validation if neither the bucketId nor bucketName are provided.',
    async () => {
      jest.spyOn(cli, 'getSettingWithDefaultValue').mockClear().mockImplementation((settingName, defaultValue) => {
        if (settingName === settingsNames.prompt) {
          return false;
  ...
  it('correctly adds planner task with title, planId, and bucketId',
    async () => {
      const options: any = {
        title: 'My Planner Task',
        planId: '8QZEH7b3wkS_bGQobscsM5gADCBb',
  ...
  it('correctly adds planner bucket with title, bucketId, planTitle, and ownerGroupName',
    async () => {
  ...
  it('correctly adds planner task with title, bucketId, planTitle, and ownerGroupId',
    async () => {
      jestUtil.restore(request.get);
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `https://graph.microsoft.com/v1.0/groups/0d0402ee-970f-4951-90b5-2f24519d2e40/planner/plans`) {
  ...
  it('correctly adds planner task with title, planId, and bucketName',
    async () => {
      jestUtil.restore(request.get);
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('correctly adds planner task with title, bucketId, planId, and assignedToUserIds',
    async () => {
      const options: any = {
        title: 'My Planner Task',
        planId: '8QZEH7b3wkS_bGQobscsM5gADCBb',
  ...
  it('correctly adds planner task with title, bucketId, planId, assignedToUserNames, and appliedCategories',
    async () => {
      jestUtil.restore(request.get);
  ...
  it('correctly adds planner task with title, bucketId, planId, assignedToUserNames, and appliedCategories split with space',
    async () => {
      jestUtil.restore(request.get);
      jestUtil.restore(request.post);
  ...
  it('correctly adds planner task with title, bucketId, planId, and description',
    async () => {
      jestUtil.restore(request.get);
  ...
  it('correctly adds planner task with title, bucketId, and rosterId',
    async () => {
      jestUtil.restore(request.get);
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('uses correct value for urgent priority', async () => {
    jestUtil.restore(request.post);
    const requestPostStub = jest.spyOn(request, 'post').mockClear().mockImplementation();
    ...
    await command.action(logger, { options: options } as any);
    assert.strictEqual(requestPostStub.mock.lastCall[0].data.priority, 1);
  });
  ...
  it('uses correct value for important priority', async () => {
    ...
    await command.action(logger, { options: options } as any);
    assert.strictEqual(requestPostStub.mock.lastCall[0].data.priority, 3);
  });
  ...
  it('uses correct value for medium priority', async () => {
    ...
    await command.action(logger, { options: options } as any);
    assert.strictEqual(requestPostStub.mock.lastCall[0].data.priority, 5);
  });
  ...
  it('uses correct value for low priority', async () => {
    ...
    await command.action(logger, { options: options } as any);
    assert.strictEqual(requestPostStub.mock.lastCall[0].data.priority, 9);
  });
