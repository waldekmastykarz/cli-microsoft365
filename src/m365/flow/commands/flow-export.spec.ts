...
describe(commands.EXPORT, () => {
  ...
  const getFakes = async (opts: any) => {
    ...
    if ((opts.url as string).indexOf(notFoundFlowName) > -1) {
      return {
        errors: [{
          "code": "ConnectionAuthorizationFailed",
          "message": `The caller with object id '${foundEnvironmentId}' does not have permission for connection '${notFoundFlowName}' under Api 'shared_logicflows'.`
    ...
    if (opts.url.match(/\/flows\/[^\?]+\?api-version\=2016-11-01/i)) {
      return {
        "id": `/providers/Microsoft.ProcessSimple/environments/Default-${foundEnvironmentId}/flows/${foundFlowName}`,
        "name": `${foundFlowName}`,
  ...
  it('exports the specified flow (debug)', async () => {
    ...
    jest.spyOn(fs, 'writeFileSync').mockClear().mockImplementation(writeFileSyncFake);

    await command.action(logger, { options: { debug: true, name: `${foundFlowName}`, environmentName: `Default-${foundEnvironmentId}`, format: 'zip' } });
  ...
  it('exports flow to zip does not contain token', async () => {
    const getRequestsStub: jest.Mock = jest.spyOn(request, 'get').mockClear().mockImplementation(getFakes);
    jest.spyOn(request, 'post').mockClear().mockImplementation(postFakes);
    jest.spyOn(fs, 'writeFileSync').mockClear().mockImplementation(writeFileSyncFake);

    await command.action(logger, { options: { debug: true, name: `${foundFlowName}`, environmentName: `Default-${foundEnvironmentId}`, format: 'zip' } });
    assert.strictEqual(getRequestsStub.mock.lastCall[0].headers['x-anonymous'], true);
  });

  it('exports the specified flow with a non zip file returned by the API (debug)',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(getFakes);
      jest.spyOn(request, 'post').mockClear().mockImplementation(postFakes);
      jest.spyOn(fs, 'writeFileSync').mockClear().mockImplementation(writeFileSyncFake);

      await command.action(logger, { options: { debug: true, name: `${nonZipFileFlowId}`, environmentName: `Default-${foundEnvironmentId}`, format: 'zip', path: './output.zip', verbose: true } });
  ...
  it('exports the specified flow in json format', async () => {
    ...
    await command.action(logger, { options: { name: `${foundFlowName}`, environmentName: `Default-${foundEnvironmentId}`, format: 'json' } });
    assert(loggerLogSpy.calledWith(`./${flowDisplayName}.json`));
  });

  it('exports the specified flow in json format with illegal characters',
    async () => {
  ...
  it('exports the specified flow in json format (debug)', async () => {
    ...
    await command.action(logger, { options: { debug: true, name: `${foundFlowName}`, environmentName: `Default-${foundEnvironmentId}`, format: 'json' } });
  ...
  it('returns ZIP file location when format specified as ZIP', async () => {
    ...
    await command.action(logger, { options: { name: `${foundFlowName}`, environmentName: `Default-${foundEnvironmentId}`, format: 'zip' } });
  ...
  it('nothing returned when path parameter is specified', async () => {
    ...
    await command.action(logger, { options: { name: `${foundFlowName}`, environmentName: `Default-${foundEnvironmentId}`, format: 'zip', path: './output.zip' } });
  ...
  it('call is made with suggestedCreationType properties when format specified as ZIP',
    async () => {
      ...
      const postRequestsStub: jest.Mock = jest.spyOn(request, 'post').mockClear().mockImplementation(postFakes);
      jest.spyOn(request, 'get').mockClear().mockImplementation(getFakes);
      jest.spyOn(fs, 'writeFileSync').mockClear().mockImplementation(writeFileSyncFake);

      await command.action(logger, { options: { name: `${foundFlowName}`, environmentName: `Default-${foundEnvironmentId}`, format: 'zip', path: './output.zip' } });
      assert.strictEqual(postRequestsStub.mock.lastCall[0].data.resources["L1BST1ZJREVSUy9NSUNST1NPRlQuRkxPVy9GTE9XUy9GMkVCOEIzNy1GNjI0LTRCMjItOTk1NC1CNUQwQ0JCMjhGOEI="].suggestedCreationType, 'Update');
      resourceIds.forEach((id) => {
        assert.strictEqual(postRequestsStub.mock.lastCall[0].data.resources[id].suggestedCreationType, 'Existing');
      });
    }
  );
  ...
  it('correctly handles no environment found', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { environmentName: `Default-${notFoundEnvironmentId}`, name: `${foundFlowName}` } } as any),
  ...
  it('correctly handles Flow not found', async () => {
    ...
    await assert.rejects(command.action(logger, { options: { environmentName: `Default-${foundEnvironmentId}`, name: notFoundFlowName } } as any),
  ...
  it('fails validation if the id is not a GUID', async () => {
    const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: 'abc' } }, commandInfo);
    assert.notStrictEqual(actual, true);
  });

  it('fails validation if format is specified as neither JSON nor ZIP',
    async () => {
      const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}`, format: 'text' } }, commandInfo);
      assert.notStrictEqual(actual, true);
    }
  );
  ...
  it('fails validation if format is specified as JSON and packageCreatedBy parameter is specified',
    async () => {
      const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}`, format: 'json', packageCreatedBy: 'abc' } }, commandInfo);
      assert.notStrictEqual(actual, true);
    }
  );
  ...
  it('fails validation if format is specified as JSON and packageDescription parameter is specified',
    async () => {
      const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}`, format: 'json', packageDescription: 'abc' } }, commandInfo);
      assert.notStrictEqual(actual, true);
    }
  );
  ...
  it('fails validation if format is specified as JSON and packageDisplayName parameter is specified',
    async () => {
      const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}`, format: 'json', packageDisplayName: 'abc' } }, commandInfo);
      assert.notStrictEqual(actual, true);
    }
  );
  ...
  it('fails validation if format is specified as JSON and packageSourceEnvironment parameter is specified',
    async () => {
      const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}`, format: 'json', packageSourceEnvironment: 'abc' } }, commandInfo);
      assert.notStrictEqual(actual, true);
    }
  );
  ...
  it('fails validation if specified path doesn\'t exist', async () => {
    ...
    const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}`, path: '/path/not/found.zip' } }, commandInfo);
    jestUtil.restore(fs.existsSync);
    assert.notStrictEqual(actual, true);
  });

  it('passes validation when the id and environment specified', async () => {
    const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}` } }, commandInfo);
    assert.strictEqual(actual, true);
  });

  it('passes validation when the id and environment specified and format set to JSON',
    async () => {
      const actual = await command.validate({ options: { environmentName: `Default-${foundEnvironmentId}`, name: `${foundFlowName}`, format: 'json' } }, commandInfo);
      assert.strictEqual(actual, true);
    }
  );
});
