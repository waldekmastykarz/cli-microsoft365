...
describe(commands.REPORT_MAILBOXUSAGEMAILBOXCOUNT, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.REPORT_MAILBOXUSAGEMAILBOXCOUNT);
  });
  ...
  it('gets the report for the last week', async () => {
    const requestStub: jest.Mock = jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
      if (opts.url === `https://graph.microsoft.com/v1.0/reports/getMailboxUsageMailboxCounts(period='D7')`) {
    ...
    assert.strictEqual(requestStub.mock.lastCall[0].url, "https://graph.microsoft.com/v1.0/reports/getMailboxUsageMailboxCounts(period='D7')");
