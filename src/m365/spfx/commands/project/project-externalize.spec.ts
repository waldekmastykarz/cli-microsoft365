...
describe(commands.PROJECT_EXTERNALIZE, () => {
  ...
  it(`correctly handles the case when .yo-rc.json exists but doesn't contain spfx project info`,
    async () => {
      ...
      assert.strictEqual(getProjectVersionSpy.mock.lastCall.returnValue, '1.8.1');
    }
  );
  ...
  it('determines the current version from .yo-rc.json when available',
    async () => {
      ...
      assert.strictEqual(getProjectVersionSpy.mock.lastCall.returnValue, '0.4.1');
    }
  );
  ...
  it('e2e: shows correct number of findings for externalizing react web part 1.8.2 project',
    async () => {
      ...
      jest.spyOn(request, 'post').mockClear().mockImplementation().resolves(JSON.stringify({ scriptType: 'module' }));
  ...
  it('returns edit suggestions', async () => {
    ...
    jest.spyOn(request, 'head').mockClear().mockImplementation().resolves();
    jest.spyOn(request, 'post').mockClear().mockImplementation().resolves(JSON.stringify({ scriptType: 'script' }));
  ...
  it('handles failures properly', async () => {
    ...
    jest.spyOn(request, 'head').mockClear().mockImplementation().resolves();
    jest.spyOn(request, 'post').mockClear().mockImplementation((options: CliRequestOptions) => {
    ...
    jest.spyOn(fs, 'writeFileSync').mockClear().mockImplementation((path, value, encoding) => {
