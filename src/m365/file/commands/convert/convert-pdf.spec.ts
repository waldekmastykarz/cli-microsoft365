...
describe(commands.CONVERT_PDF, () => {
  ...
  beforeAll(() => {
    ...
    unlinkSyncStub = jest.spyOn(fs, 'unlinkSync').mockClear().mockReturnValue();
  ...
  beforeEach(() => {
    ...
    jest.spyOn(fs, 'createWriteStream').mockClear().mockReturnValue(pdfConvertWriteStream as any);
    setTimeout(() => {
  ...
  describe('app-only auth', () => {
    ...
    it('converts file from root site collection, root site, default doc lib, root folder to a local file',
      async () => {
        ...
        jest.spyOn(request, 'post').mockClear().mockImplementation().rejects(new Error('Issued POST request'));
        jest.spyOn(request, 'put').mockClear().mockImplementation().rejects(new Error('Issued PUT request'));
        jest.spyOn(request, 'delete').mockClear().mockImplementation().rejects(new Error('Issue DELETE request'));
    ...
    it('converts file from root site collection, root site, default doc lib, root folder to a local file (debug)',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from root site collection, root site, default doc lib, sub folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from root site collection, root site, custom doc lib, root folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from root site collection, sub site, default doc lib, root folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from root site collection, sub site, default doc lib, sub folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from root site collection, sub site, custom doc lib, root folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from One Drive for Business, default doc lib, root folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from One Drive for Business, default doc lib, sub folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from non-root site collection, root site, default doc lib, root folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from non-root site collection, root site, default doc lib, sub folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts file from non-root site collection, sub site, default doc lib, root folder to a local file',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts local file to a local PDF file, removes the temporarily uploaded file after conversion succeeded',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('converts local file to a PDF, uploads converted file to the root site collection, root site, default document library, root folder, removes the temporarily uploaded file and the temporary local file after conversion succeeded',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it(`returns error when the specified document library doesn't exist`,
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it(`returns error when the specified remote file doesn't exist`,
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it(`returns error when can't write to the specified local file`,
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
        ...
        jestUtil.restore(fs.createWriteStream);
        const invalidStream = new PassThrough();
        jest.spyOn(fs, 'createWriteStream').mockClear().mockReturnValue(invalidStream as any);
        setTimeout(() => {
    ...
    it('returns error when resolving Graph URL for the converted file to be uploaded failed',
      async () => {
        let i: number = 0;
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          ++i;
    ...
    it('returns error when creating Graph upload session for the converted file to be uploaded failed',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('returns error when uploading the converted file failed', async () => {
      ...
      jest.spyOn(request, 'put').mockClear().mockImplementation().rejects({
        "error": {
          "message": "An error has occurred"
        }
      });
    ...
    it('returns error when after conversion removing the temporarily uploaded file failed',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
    ...
    it('returns error when removing the temporary local file after conversion failed',
      async () => {
        jest.spyOn(request, 'get').mockClear().mockImplementation(async opts => {
          const url: string = opts.url as string;
        ...
        jest.spyOn(fs, 'readFileSync').mockClear().mockReturnValue('abc');
        jestUtil.restore(fs.unlinkSync);
        unlinkSyncStub = jest.spyOn(fs, 'unlinkSync').mockClear().mockImplementation().throws(new Error('An error has occurred'));
