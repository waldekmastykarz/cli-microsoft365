...
describe(commands.CDN_POLICY_LIST, () => {
  ...
  it('retrieves the policies for the public CDN when type set to Public',
    async () => {
  ...
  it('retrieves the policies for the private CDN when type set to Private',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf('/_api/contextinfo') > -1) {
          if (opts.headers &&
  ...
  it('retrieves the policies for the public CDN when no type set',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf('/_api/contextinfo') > -1) {
          if (opts.headers &&
  ...
  it('correctly handles an error when retrieving tenant CDN policies',
    async () => {
