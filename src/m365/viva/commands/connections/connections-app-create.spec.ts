...
describe(commands.CONNECTIONS_APP_CREATE, () => {
  ...
  it('creates app package for the specified communication site (root site)',
    async () => {
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async () => {
        return {
      ...
      const admZipMockAddFileSpy = jest.spyOn(admZipMock, 'addFile').mockClear();
      const admZipMockAddLocalFileSpy = jest.spyOn(admZipMock, 'addLocalFile').mockClear();
      const admZipMockWriteZipSpy = jest.spyOn(admZipMock, 'writeZip').mockClear();
  ...
  it('creates app package for the specified communication site (/sites)',
    async () => {
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async () => {
        return {
          stdout: JSON.stringify({
  ...
  it('creates app package for the specified communication site (/teams + query string)',
    async () => {
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async () => {
        return {
          stdout: JSON.stringify({
  ...
  it(`fails with an error if the specified site doesn't exist`, async () => {
    jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation().rejects({
  ...
  it(`fails with an error if the specified site doesn't exist (debug)`,
    async () => {
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation().rejects({
        error: '404 - FILE NOT FOUND',
  ...
  it('fails with an error if the specified site is not a communication site',
    async () => {
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async () => {
        return {
          stdout: JSON.stringify({
  ...
  it(`fails with an error if creating the zip file failed`, async () => {
    ...
    jest.spyOn(admZipMock, 'writeZip').mockClear().mockImplementation(() => {
  ...
  it(`passes validation if all arguments are correct`, async () => {
    jest.spyOn(fs, 'existsSync').mockClear().mockImplementation((path) => path.toString().indexOf('.zip') < 0);
    const actual = await command.validate({
      options: {
        portalUrl: 'https://contoso.sharepoint.com',
        name: 'Contoso',
