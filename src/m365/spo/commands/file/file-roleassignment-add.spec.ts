...
describe(commands.FILE_ROLEASSIGNMENT_ADD, () => {
  ...
  it('fails validation if the principalId option is not a valid number',
    async () => {
  ...
  it('fails validation if the roleDefinitionId option is not a valid number',
    async () => {
  ...
  it('correctly adds role assignment specifying principalId and role definition name',
    async () => {
  ...
  it('correctly adds role assignment specifying principalId and role definition name, retrieving file by the ID',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url as string === `https://contoso.sharepoint.com/sites/project-x/_api/web/GetFileByServerRelativePath(DecodedUrl='%2Fsites%2Fproject-x%2Fdocuments%2FTest1.docx')/ListItemAllFields/roleassignments/addroleassignment(principalid='10',roledefid='1073741827')`) {
          return;
  ...
  it('correctly adds role assignment specifying upn and role definition id',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
  ...
  it('correctly adds role assignment specifying groupName and role definition id',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
  ...
  it('correctly handles error when role definition does not exist',
    async () => {
      ...
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async (command) => {
        if (command === spoRoleDefinitionListCommand) {
          throw error;
        }
