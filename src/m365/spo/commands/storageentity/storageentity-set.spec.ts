...
describe(commands.STORAGEENTITY_SET, () => {
  ...
  it('sets tenant property', async () => {
    ...
    assert.strictEqual((postStub.mock.lastCall[0].headers as any)['X-RequestDigest'], 'ABC');
  ...
  it('sets tenant property without description and comment (debug)',
    async () => {
      const postStub: jest.Mock = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf('/_vti_bin/client.svc/ProcessQuery') > -1) {
          return JSON.stringify([{ "SchemaVersion": "15.0.0.0", "LibraryVersion": "16.0.7018.1204", "ErrorInfo": null, "TraceCorrelationId": "4456299e-d09e-4000-ae61-ddde716daa27" }, 31, { "IsNull": false }, 33, { "IsNull": false }, 35, { "IsNull": false }]);
  ...
  it('correctly handles a generic error when setting tenant property',
    async () => {
      const postStub: jest.Mock = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf('/_vti_bin/client.svc/ProcessQuery') > -1) {
  ...
  it('correctly handles access denied error when setting tenant property',
    async () => {
      const postStub: jest.Mock = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf('/_vti_bin/client.svc/ProcessQuery') > -1) {
          return JSON.stringify([
  ...
  it('handles promise rejection', async () => {
    jest.spyOn(spo, 'getSpoAdminUrl').mockClear().mockImplementation().rejects(new Error('error'));
