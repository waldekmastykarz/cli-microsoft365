...
describe(commands.LIST_CONTENTTYPE_REMOVE, () => {
  ...
  it('prompts before removing content type from list when confirmation argument not passed (listId)',
    async () => {
  ...
  it('prompts before removing content type from list when confirmation argument not passed (listTitle)',
    async () => {
      await command.action(logger, {
        options: {
          listTitle: listTitle,
  ...
  it('aborts removing content type from list when prompt not confirmed',
    async () => {
  ...
  it('removes content type from list when listId option is passed and prompt confirmed (debug)',
    async () => {
  ...
  it('removes content type from list when listTitle option is passed and prompt confirmed (debug)',
    async () => {
      const postStub = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
  ...
  it('removes content type from list when listUrl option is passed and prompt confirmed (debug)',
    async () => {
      const postStub = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
  ...
  it('removes content type from list when listUrl option is passed and prompt confirmed',
    async () => {
      const postStub = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === 'https://contoso.sharepoint.com/_api/web/GetList(\'%2Fsites%2Fdocuments\')/ContentTypes(\'0x010109010053EE7AEB1FC54A41B4D9F66ADBDC312A\')') {
          if (opts.headers &&
  ...
  it('removes content type from list when listId option is passed and prompt confirmed',
    async () => {
      const postStub = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === 'https://contoso.sharepoint.com/_api/web/lists(guid\'dfddade1-4729-428d-881e-7fedf3cae50d\')/ContentTypes(\'0x010109010053EE7AEB1FC54A41B4D9F66ADBDC312A\')') {
          if (opts.headers &&
  ...
  it('removes content type from list when listTitle option is passed and prompt confirmed',
    async () => {
      const postStub = jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === 'https://contoso.sharepoint.com/_api/web/lists/getByTitle(\'Documents\')/ContentTypes(\'0x010109010053EE7AEB1FC54A41B4D9F66ADBDC312A\')') {
          if (opts.headers &&
