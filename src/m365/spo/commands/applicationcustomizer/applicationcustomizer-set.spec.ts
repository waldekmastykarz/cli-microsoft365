...
describe(commands.APPLICATIONCUSTOMIZER_SET, () => {
  ...
  const defaultUpdateCallsStub = (): jest.Mock => {
  ...
  it('handles error when multiple application customizer with the specified title found',
    async () => {
      jest.spyOn(cli, 'getSettingWithDefaultValue').mockClear().mockImplementation((settingName, defaultValue) => {
        if (settingName === settingsNames.prompt) {
          return false;
  ...
  it('handles error when multiple application customizer with the specified clientSideComponentId found',
    async () => {
      jest.spyOn(cli, 'getSettingWithDefaultValue').mockClear().mockImplementation((settingName, defaultValue) => {
        if (settingName === settingsNames.prompt) {
          return false;
  ...
  it('handles selecting single result when multiple application customizers with the specified name found and cli is set to prompt',
    async () => {
      ...
      const updateCallsSpy: jest.Mock = defaultUpdateCallsStub();
  ...
  it('should update the application customizer from the site by its ID',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if (opts.url?.startsWith('https://contoso.sharepoint.com/_api/') && opts.url?.endsWith(`/UserCustomActions(guid'${id}')`)) {
  ...
  it('should update the application customizer from the site collection by its ID',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if (opts.url?.startsWith('https://contoso.sharepoint.com/_api/') && opts.url?.endsWith(`/UserCustomActions(guid'${id}')`)) {
  ...
  it('should update the application customizer from the site by its clientSideComponentId',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
