...
describe(commands.TENANT_COMMANDSET_ADD, () => {
  ...
  it('adds a tenant-wide ListView Command Set for the SitePages library',
    async () => {
      let executeCommandCalled = false;
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async (command, args): Promise<any> => {
        if (command === spoListItemListCommand) {
  ...
  it('adds a tenant-wide ListView Command Set to a specific webtemplate and location including clientSideComponentProperties',
    async () => {
      let executeCommandCalled = false;
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async (command, args): Promise<any> => {
        if (command === spoListItemListCommand) {
  ...
  it('throws an error when the manifest of a specific client side component is not of type ListView Command Set',
    async () => {
      jest.spyOn(Cli, 'executeCommandWithOutput').mockClear().mockImplementation(async (command, args): Promise<any> => {
        if (command === spoListItemListCommand) {
          if (args.options.listUrl === `${urlUtil.getServerRelativeSiteUrl(appCatalogUrl)}/Lists/ComponentManifests`) {
