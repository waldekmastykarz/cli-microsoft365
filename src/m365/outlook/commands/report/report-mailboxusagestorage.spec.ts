...
describe(commands.REPORT_MAILBOXUSAGESTORAGE, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.REPORT_MAILBOXUSAGESTORAGE);
  });
  ...
  it('gets the report for the last week', async () => {
    const requestStub: jest.Mock = jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
      if (opts.url === `https://graph.microsoft.com/v1.0/reports/getMailboxUsageStorage(period='D7')`) {
        return `Report Refresh Date,Storage Used (Byte),Report Date,Report Period`;
      }
    ...
    assert.strictEqual(requestStub.mock.lastCall[0].url, "https://graph.microsoft.com/v1.0/reports/getMailboxUsageStorage(period='D7')");
