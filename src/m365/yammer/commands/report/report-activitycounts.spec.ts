...
import command from './report-activitycounts.js';
...
describe(commands.REPORT_ACTIVITYCOUNTS, () => {
  ...
  it('gets the report for the last week', async () => {
    const requestStub: jest.Mock = jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
      if (opts.url === `https://graph.microsoft.com/v1.0/reports/getYammerActivityCounts(period='D7')`) {
    ...
    assert.strictEqual(requestStub.mock.lastCall[0].url, "https://graph.microsoft.com/v1.0/reports/getYammerActivityCounts(period='D7')");
