...
describe(commands.RUN_CANCEL, () => {
  ...
  it('prompts before cancelling the specified Microsoft FlowName when confirm option not passed',
    async () => {
  ...
  it('aborts cancelling the specified Microsoft FlowName when confirm option not passed and prompt not confirmed',
    async () => {
  ...
  it('cancels the specified Microsoft Flow without confirmation prompt',
    async () => {
  ...
  it('cancels the specified Microsoft FlowName when prompt confirmed',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `https://management.azure.com/providers/Microsoft.ProcessSimple/environments/Default-eff8592e-e14a-4ae8-8771-d96d5c549e1c/flows/0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac72/runs/08585981115186985105550762687CU161/cancel?api-version=2016-11-01`) {
          return { statusCode: 200 };
  ...
  it('correctly handles no environmentName found without prompting when confirm specified',
    async () => {
  ...
  it('correctly handles no environmentName found when prompt confirmed',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation().rejects({
        "error": {
          "code": "EnvironmentAccessDenied",
  ...
  it('correctly handles specified Microsoft FlowName not found when prompt confirmed',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation().rejects({
        "error": {
  ...
  it('correctly handles specified Microsoft FlowName not found without prompting when confirm specified',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation().rejects({
        "error": {
          "code": "ConnectionAuthorizationFailed",
  ...
  it('correctly handles specified Microsoft FlowName run not found when prompt confirmed',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation().rejects({
        "error": {
  ...
  it('correctly handles specified Microsoft FlowName run not found without prompting when confirm specified',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation().rejects({
        "error": {
          "code": "AzureResourceManagerRequestFailed",
