...
describe(commands.M365GROUP_RECYCLEBINITEM_CLEAR, () => {
  ...
  it('clears the recycle bin items without prompting for confirmation when --force option specified',
    async () => {
  ...
  it('clears the recycle bin items when deleted items data is served in pages and --force option specified',
    async () => {
      const deleteStub = jest.spyOn(request, 'delete').mockClear().mockImplementation().resolves();
  ...
  it('does not call delete when there are no items in the M365 group recycle bin',
    async () => {
  ...
  it('prompts before clearing the M365 Group recycle bin items when --force option is not passed',
    async () => {
  ...
  it('aborts clearing the M365 Group recyclebin items when prompt not confirmed',
    async () => {
  ...
  it('aborts clearing the recycle bin items when prompt not confirmed (debug)',
    async () => {
      const deleteSpy = jest.spyOn(request, 'delete').mockClear();
      jestUtil.restore(Cli.prompt);
      jest.spyOn(Cli, 'prompt').mockClear().mockImplementation(async () => (
  ...
  it('clears the M365 Group recycle bin items when prompt is confirmed',
    async () => {
      const deleteStub = jest.spyOn(request, 'delete').mockClear().mockImplementation().resolves();
  ...
  it('clears the M365 Group recycle bin items when prompt is confirmed (debug)',
    async () => {
      const deleteStub = jest.spyOn(request, 'delete').mockClear().mockImplementation().resolves();
