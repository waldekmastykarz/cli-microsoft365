...
describe(commands.CONTENTTYPE_ADD, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.CONTENTTYPE_ADD);
  });
  ...
  it('creates site content type with description and group (debug)',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `https://contoso.sharepoint.com/sites/sales/_vti_bin/client.svc/ProcessQuery` &&
  ...
  it('creates list retrieved by id content type with description',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if (opts.url === 'https://contoso.sharepoint.com/sites/sales/_api/web?$select=Id') {
          return { Id: '276f6d32-f43b-4b26-ada6-7aa9d5bcab6a' };
  ...
  it('creates list retrieved by url content type with description',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('correctly handles error when a content with the specified name already exists',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `https://contoso.sharepoint.com/sites/sales/_vti_bin/client.svc/ProcessQuery`) {
          return JSON.stringify([
