...
import command from './report-groupsactivitycounts.js';

describe(commands.REPORT_GROUPSACTIVITYCOUNTS, () => {
  let log: string[];
  let logger: Logger;
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.REPORT_GROUPSACTIVITYCOUNTS);
  });
  ...
  it('gets the report for the last week', async () => {
    ...
    assert.strictEqual(requestStub.mock.lastCall[0].url, "https://graph.microsoft.com/v1.0/reports/getYammerGroupsActivityCounts(period='D7')");
