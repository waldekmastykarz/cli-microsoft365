...
describe('AppCommand', () => {
  ...
  it('returns error if .m365rc.json file not found in the current directory',
    async () => {
  ...
  it(`returns error if the .m365rc.json file contents couldn't be parsed`,
    async () => {
      jest.spyOn(fs, 'existsSync').mockClear().mockReturnValue(true);
      jest.spyOn(fs, 'readFileSync').mockClear().mockReturnValue('{');
  ...
  it(`returns error if the .m365rc.json file is empty doesn't contain any apps`,
    async () => {
      jest.spyOn(fs, 'existsSync').mockClear().mockReturnValue(true);
  ...
  it(`returns error if the specified appId not found in the .m365rc.json file`,
    async () => {
      jest.spyOn(fs, 'existsSync').mockClear().mockReturnValue(true);
      jest.spyOn(fs, 'readFileSync').mockClear().mockReturnValue(JSON.stringify({
        apps: [
  ...
  it(`prompts to choose an app when multiple apps found in .m365rc.json and no appId specified`,
    async () => {
      jest.spyOn(fs, 'existsSync').mockClear().mockReturnValue(true);
      jest.spyOn(fs, 'readFileSync').mockClear().mockReturnValue(JSON.stringify({
        apps: [
      ...
      const cliPromptStub = jest.spyOn(Cli, 'handleMultipleResultsFound').mockClear().mockImplementation(async () => (
        { appIdIndex: 0 }
      ));
  ...
  it(`uses app selected by the user in the prompt`, async () => {
    ...
    jest.spyOn(Cli, 'handleMultipleResultsFound').mockClear().mockImplementation().resolves({ appIdIndex: 1 });
    jest.spyOn(Command.prototype, 'action').mockClear().mockImplementation().resolves();
    ...
    try {
      ...
      jestUtil.restore(Command.prototype.action);
    }
  });
