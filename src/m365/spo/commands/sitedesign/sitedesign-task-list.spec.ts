...
describe(commands.SITEDESIGN_TASK_LIST, () => {
  ...
  it('gets information about site designs scheduled for execution on the specified site',
    async () => {
  ...
  it('gets information about site designs scheduled for execution on the specified site (debug)',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf(`/_api/Microsoft.Sharepoint.Utilities.WebTemplateExtensions.SiteScriptUtility.GetSiteDesignTasks`) > -1) {
          return {
