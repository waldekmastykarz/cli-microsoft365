...
describe(commands.REPORT_MAILACTIVITYCOUNTS, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.REPORT_MAILACTIVITYCOUNTS);
  });
  ...
  it('gets the report for the last week', async () => {
    const requestStub: jest.Mock = jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
      if (opts.url === `https://graph.microsoft.com/v1.0/reports/getEmailActivityCounts(period='D7')`) {
        return `RReport Refresh Date,Send,Receive,Read,Report Date,Report Period`;
      }
    ...
    assert.strictEqual(requestStub.mock.lastCall[0].url, "https://graph.microsoft.com/v1.0/reports/getEmailActivityCounts(period='D7')");
