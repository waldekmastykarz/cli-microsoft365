...
describe(commands.TERM_SET_LIST, () => {
  ...
  it('lists taxonomy term sets from the term group specified using id',
    async () => {
  ...
  it('lists taxonomy term sets from the term group specified using name',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === 'https://contoso-admin.sharepoint.com/_vti_bin/client.svc/ProcessQuery' &&
          opts.headers &&
  ...
  it('lists taxonomy term sets from the specified site collection with specified term group using name',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
  ...
  it('lists taxonomy term sets with all properties when output is JSON',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if (opts.url === 'https://contoso-admin.sharepoint.com/_vti_bin/client.svc/ProcessQuery' &&
          opts.headers &&
  ...
  it('correctly handles error when retrieving taxonomy term sets',
    async () => {
  ...
  it('correctly handles error when the specified term group id doesn\'t exist',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation().resolves(JSON.stringify([
        {
  ...
  it('correctly handles error when the specified term group name doesn\'t exist',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation().resolves(JSON.stringify([
        {
          "SchemaVersion": "15.0.0.0", "LibraryVersion": "16.0.8112.1217", "ErrorInfo": {
