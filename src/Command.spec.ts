...
describe('Command', () => {
  ...
  beforeAll(() => {
    ...
    jest.spyOn(telemetry, 'trackEvent').mockClear().mockImplementation((commandName) => {
  ...
  it('shows deprecation warning when command executed using the deprecated name',
    async () => {
      try {
        ...
        expect(loggerLogToStderrSpy).toHaveBeenCalledWith(chalk.yellow(`Command 'mc1' is deprecated. Please use 'mock-command' instead`));
      }
      catch (err: any) {
        assert.fail(err);
      }
  ...
  it('logs command alias in the telemetry when command alias used',
    async () => {
      cli.currentCommandName = 'mc1';
      const mock = new MockCommand1();
      await mock.action(logger, { options: {} });
  ...
  it('logs empty command name in telemetry when command called using something else than name or alias',
    async () => {
  ...
  it('handles option with @meid token and spaces', async () => {
    ...
    const commandCommandActionSpy: jest.SpyInstance = jest.spyOn(command, 'commandAction').mockClear();
    jest.spyOn(accessToken, 'getUserIdFromAccessToken').mockClear().mockReturnValue('f3e59491-fc1a-47cc-a1f0-95ed45983717');

    await command.action(logger, { options: { option1: '@Meid ' } });
    assert.deepStrictEqual(commandCommandActionSpy.mock.lastCall[1].options.option1, 'f3e59491-fc1a-47cc-a1f0-95ed45983717');
  });
  ...
  it('handles option with @meusername token and spaces', async () => {
    ...
    const command = new MockCommand3();
    const commandCommandActionSpy: jest.SpyInstance = jest.spyOn(command, 'commandAction').mockClear();
    jest.spyOn(accessToken, 'getUserNameFromAccessToken').mockClear().mockReturnValue('admin@contoso.onmicrosoft.com');

    await command.action(logger, { options: { option1: '@MeUsername ' } });
    assert.deepStrictEqual(commandCommandActionSpy.mock.lastCall[1].options.option1, 'admin@contoso.onmicrosoft.com');
  });
  ...
  it('handles @meid with application permissions', async () => {
    ...
    const command = new MockCommand3();
    jest.spyOn(accessToken, 'isAppOnlyAccessToken').mockClear().mockImplementation(() => { return true; });
