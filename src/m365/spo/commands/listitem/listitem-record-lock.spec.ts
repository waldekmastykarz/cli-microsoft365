...
describe(commands.LISTITEM_RECORD_LOCK, () => {
  ...
  it('has correct name', () => {
    assert.strictEqual(command.name, commands.LISTITEM_RECORD_LOCK);
  });
  ...
  it('locks a list item based on listUrl (debug)', async () => {
    jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
      if (opts.url === `https://contoso.sharepoint.com/_api/SP.CompliancePolicy.SPPolicyStoreProxy.LockRecordItem()`) {
        return;
      }
  ...
  it('locks a list item based on listTitle', async () => {
  ...
  it('locks a list item based on listId', async () => {
    jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
