...
describe(commands.DOCTOR, () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(process, 'platform').mockClear().mockImplementation().value('linux');
  });
  ...
  it('passes all checks for SPFx v1.11 project when all requirements met',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.22.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.11 project when all requirements met (debug)',
    async () => {
      const sandbox = sinon.createSandbox();
  ...
  it('passes all checks for SPFx v1.11 generator installed locally when all requirements met (debug)',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.11 generator installed globally when all requirements met',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.11 generator installed locally when all requirements met',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.10 project when all requirements met',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.10 project when all requirements met (debug)',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.10 generator installed locally when all requirements met',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.10 generator installed locally when all requirements met (debug)',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.10 generator installed globally when all requirements met',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes all checks for SPFx v1.11 generator installed globally, SPFx version specified through args, when all requirements met',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('fails with error when SPFx not found, version passed explicitly',
    async () => {
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
        const packageName: string = file.split(' ')[2];
        switch (packageName) {
  ...
  it('passes Node check when version meets single range prerequisite',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('passes Node check when version meets double range prerequisite',
    async () => {
      const sandbox = sinon.createSandbox();
  ...
  it('fails Node check when version does not meet single range prerequisite',
    async () => {
      const sandbox = sinon.createSandbox();
  ...
  it('fails Node check when version does not meet double range prerequisite',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v12.0.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
  ...
  it('determines the current version from .yo-rc.json when spfxVersion not passed',
    async () => {
  ...
  it('determines the current version from .yo-rc.json when available when spfxVersion is passed',
    async () => {
      const originalExistsSync = fs.existsSync;
      jest.spyOn(fs, 'existsSync').mockClear().mockImplementation((path) => {
        if (path.toString().endsWith('.yo-rc.json')) {
  ...
  it('returns error when used with an unsupported version of spfx',
    async () => {
      const sandbox = sinon.createSandbox();
      sandbox.stub(process, 'version').value('v10.18.0');
      jest.spyOn(child_process, 'exec').mockClear().mockImplementation((file, callback: any) => {
