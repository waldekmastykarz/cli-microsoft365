...
describe(commands.EVENTRECEIVER_GET, () => {
  ...
  it('throws error when multiple eventreceivers with the same name were found',
    async () => {
  ...
  it('handles selecting single result when multiple eventreceiver with the specified name found and cli is set to prompt',
    async () => {
      const multipleEventreceiversResponse = {
        value: [
          { ReceiverId: '69703efe-4149-ed11-bba2-000d3adf7537' },
      ...
      jest.spyOn(Cli, 'handleMultipleResultsFound').mockClear().mockImplementation().resolves(eventReceiverResponseJson);
  ...
  it('retrieves list event receiver retrieved by list title using name as option',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf(`/_api/web/lists/getByTitle('Documents')/eventreceivers?$filter=receivername eq 'PnP Test Receiver'`) > -1) {
          return eventReceiverValue;
  ...
  it('retrieves list event receivers queried by url using name as option',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('retrieves list event receivers queried by list id using name as option',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('retrieves list event receiver retrieved by list title using id as option',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('retrieves list event receivers queried by url using id as option',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('retrieves list event receivers queried by list id using id as option',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
