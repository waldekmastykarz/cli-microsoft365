...
describe(commands.HUBSITE_LIST, () => {
  ...
  it('correctly retrieves the associated sites in batches (debug)',
    async () => {
      // Cast the command class instance to any so we can set the private
      // property 'batchSize' to a small number for easier testing
      const newBatchSize = 3;
  ...
  it('lists hub sites, including associated sites, with all properties for JSON output',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation().resolves({
        value: [
          {
  ...
  it('correctly handles empty result while retrieving associated sites in batches',
    async () => {
      // Cast the command class instance to any so we can set the private
      // property 'batchSize' to a small number for easier testing
      const newBatchSize = 3;
  ...
  it('correctly handles OData error when retrieving hub sites', async () => {
    jest.spyOn(request, 'get').mockClear().mockImplementation().rejects({ error: { 'odata.error': { message: { value: 'An error has occurred' } } } });

    await assert.rejects(command.action(logger, { options: {} } as any),
      new CommandError('An error has occurred'));
  });

  it('correctly handles error in the first batch of associated sites',
    async () => {
      const error = {
        error: {
          'odata.error': {
  ...
  it('correctly handles error in a subsequent batch of associated sites',
    async () => {
      const error = {
        error: {
          'odata.error': {
