...
describe(commands.FILE_RENAME, () => {
  ...
  it('forcefully renames file from a non-root site in the root folder of a document library when a file with the same name exists (or it doesn\'t?)',
    async () => {
      jest.spyOn(Cli, 'executeCommand').mockClear().mockImplementation().resolves();
  ...
  it('renames file from a non-root site in the root folder of a document library when a file with the same name doesn\'t exist',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string) === 'https://contoso.sharepoint.com/sites/portal/_api/web/GetFileByServerRelativePath(DecodedUrl=\'%2Fsites%2Fportal%2FShared%20Documents%2Fabc.pdf\')/ListItemAllFields/ValidateUpdateListItem()') {
          return renameValue;
  ...
  it('continues if file cannot be recycled because it does not exist',
    async () => {
      ...
      jest.spyOn(Cli, 'executeCommand').mockClear().mockImplementation().rejects(fileDeleteError);
