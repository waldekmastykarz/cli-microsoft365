...
describe(commands.APPPAGE_ADD, () => {
  ...
  it('fails to create a single-part app page if creating page failed',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf(`_api/sitepages/Pages/CreateAppPage`) > -1) {
  ...
  it('fails to create a single-part app page if retrieving the created page failed',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf(`_api/sitepages/Pages/CreateAppPage`) > -1) {
          return { value: "SitePages/lp4blf70.aspx" };
  ...
  it('fails to create a single-part app page if updating the created page failed',
    async () => {
      jest.spyOn(request, 'post').mockClear().mockImplementation(async (opts) => {
        if ((opts.url as string).indexOf(`_api/sitepages/Pages/CreateAppPage`) > -1) {
          return { value: "SitePages/lp4blf70.aspx" };
