...
describe(commands.LIST_VIEW_GET, () => {
  ...
  it('correctly handles error when the specified view doesn\'t exist',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation().rejects({
        error: {
          "odata.error": {
  ...
  it('should successfully get the list view with specified its Id',
    async () => {
      ...
      assert.strictEqual(loggerLogSpy.mock.lastCall[0].Id, 'ba84217c-8561-4234-aa95-265081e74be9');
    }
  );

  it('should successfully get the list view with specified its name',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
      ...
      assert.strictEqual(loggerLogSpy.mock.lastCall[0].Title, 'All Items');
    }
  );

  it('should successfully get the list view with specified its name and list id',
    async () => {
      jest.spyOn(request, 'get').mockClear().mockImplementation(async (opts) => {
  ...
  it('passes validation when id and listId specified and valid GUIDs',
    async () => {
