...
describe(commands.FILE_VERSION_REMOVE, () => {
  ...
  it('prompts before removing the specified version when confirm option not passed',
    async () => {
  ...
  it('aborts removing the specified version when confirm option not passed and prompt not confirmed',
    async () => {
  ...
  it('deletes a version from a file with the fileUrl options asking to confirm',
    async () => {
      jest.spyOn(request, 'delete').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `${validWebUrl}/_api/web/GetFileByServerRelativePath(DecodedUrl='${formatting.encodeQueryParameter(validFileUrl)}')/versions/DeleteByLabel('${validLabel}')`) {
          return { statusCode: 200 };
  ...
  it('deletes a version from a file with the fileId options asking to confirm',
    async () => {
      jest.spyOn(request, 'delete').mockClear().mockImplementation(async (opts) => {
        if (opts.url === `${validWebUrl}/_api/web/GetFileById('${validFileId}')/versions/DeleteByLabel('${validLabel}')`) {
          return { statusCode: 200 };
